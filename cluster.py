...
class Male(me.Document):
    ...
    user_id = me.StringField(unique=True)
    status = me.IntField
...
def getUser(user_id):
    qMale = Male.objects(user_id = user_id)
    qFemale = Female.objects(user_id = user_id)
    if qMale:
        return Male.objects.get(user_id =user_id)
    elif qFemale:
        return Female.objects.get(user_id =user_id)
    else :
        return None
