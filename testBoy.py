...
class TestFunction(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for function in inspect.getmembers(cls, predicate=inspect.isfunction):
            if function[0].find('test_') != -1 :
                name = function[0]
                if cluster.Member.getUser(name):cluster.Member.getUser(name).delete()
                if cluster.Member.getUser(name+"G"):cluster.Member.getUser(name+"G").delete()
                if cluster.Member.getDate(name):cluster.Member.getDate(name).delete()
                if cluster.Member.getDate(name+"G"):cluster.Member.getDate(name+"G").delete()
    ...
    def test_約她(self):
        ...
        t_memeber = cluster.Member.getUser(inspect.currentframe().f_code.co_name)
        self.assertEqual(t_memeber.status, 110)
        t_memeber.delete()
        t_date = cluster.Member.getDate(inspect.currentframe().f_code.co_name+"G")
    ...
    def test_約會成功(self):
        ...
        t_memeber.delete()
        t_memeber = cluster.Member.getUser(inspect.currentframe().f_code.co_name+"G")
        self.assertEqual(t_memeber.status, 100)
        t_memeber.delete()
        t_date.delete()
