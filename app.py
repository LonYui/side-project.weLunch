...
def webhook():
    ...
    if Etype not in ["postback", "message"]:
        return
    ...
    if Etype == "postback":
        ...
        if status == 100:
            ...
            replytext = date.ST_Dstatus(reqstext=userId,
                                        userId=userId,
            ...
            replytext = date.ST_Dstatus(reqstext=reqstext,
                                        userId=userId,
                                        token=token, client=client)
            return replytext
    ...
    if not user:
        if reqstext in ["開始使用", "去註冊約她"]:
            ...
            if token != userId:
                client.reply_message(
                    token, [TextSendMessage(text=replytext),
            ...
            column = template.CarouselColumn(
                text=replytext, actions=[action1, action2])
            carouse = template.CarouselTemplate(columns=[column])
            if token != userId:
                client.reply_message(
    ...
    if status == 3:
        ...
        user.status += 1
        replytext = "您是" + user.birthDate.isoformat() \
            ...
            if token != userId:
                client.reply_message(
                    token, [template.TemplateSendMessage(
                        template=carouse, alt_text="broke")])
            return ""
        replytext = date.ST_Dstatus(
            reqstext=reqstext, userId=userId,
            token=token, client=client)
        return replytext
    user.save()
    if token != userId:
